...
def create_nuscenes_annotations(root_path, info_prefix, version='v1.0-trainval'):
    ...
    pts_dir = osp.join(root_path, 'project_points')
    if not osp.exists(pts_dir):
        os.mkdir(pts_dir)
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, pts_dir)
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, pts_dir):
    train_nusc_infos = []
    val_nusc_infos = []
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        for cam in camera_types:
            ...
            pts_path = get_proj_pts(lidar_points, cam_info, pts_dir)
            cam_info.update(pts_path=pts_path)
...
def get_proj_pts(lidar_points, cam_info, save_dir, z_clip=0.1):
    ...
    in_canvas = ((proj_cam_uv >= 0) & (
            proj_cam_uv < np.array(imsize, dtype=proj_cam_uv.dtype)
    )).min(axis=1)
    ...
    # get UVZ points
    cam_pts_uvz = np.concatenate([proj_cam_uv, cam_points[:, 2:]], axis=-1)

    # save data
    filename = osp.splitext(osp.basename(cam_info['data_path']))[0]
    data_path = osp.join(save_dir, filename) + '__pts.pkl'
    mmcv.dump(dict(oc_list=oc_list, uv_list=uv_list, cam_pts_uvz=cam_pts_uvz), data_path)
