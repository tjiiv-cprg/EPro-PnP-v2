...
def project_to_image_r_mat(
    ...
    # clip to border
    x2d_min = -allowed_border  # Number
    x2d_max = img_shapes[..., None, [1, 0]] + allowed_border  # (*, 1, 2)
    if return_clip_mask:
...
def bboxes_3d_to_2d(bbox_3d, cam_intrinsic, imsize, z_clip=0.1, min_size=4.0, clip=False):
    ...
    if bs > 0:
        ...
        pts_2d = pts_2d[..., :2] / pts_2d[..., 2:].clamp(min=z_clip)  # (bs, 8, 2)
        in_canvas = in_front
        if clip:
        ...
        imsize_wh = torch.flip(imsize, [-1])
        pts_2d[not_in_canvas] = imsize_wh[:, None].expand(-1, 8, -1)[not_in_canvas]
        x0y0 = pts_2d.min(dim=1)[0].clamp(min=0)  # (bs, 2)
        pts_2d[not_in_canvas] = 0
        x1y1 = torch.minimum(pts_2d.max(dim=1)[0], imsize_wh)
