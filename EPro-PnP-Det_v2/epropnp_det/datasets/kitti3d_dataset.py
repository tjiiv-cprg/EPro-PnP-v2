...
class KITTI3DDataset(CustomDataset):
    ...
    def __init__(self,
                 ...
                 meta_prefix='img_metas/',
                 coord_3d_prefix='project_points/',
                 calib_prefix='calib/',
                 calib_cam=2,
                 max_truncation=0.5,
                 max_occlusion=2,
        ...
        if data_root is not None:
            if not (self.label_prefix is None or osp.isabs(self.label_prefix)):
                self.label_prefix = osp.join(data_root, self.label_prefix)
            if not (self.meta_prefix is None or osp.isabs(self.meta_prefix)):
                self.meta_prefix = osp.join(data_root, self.meta_prefix)
            if not (self.calib_prefix is None or osp.isabs(self.calib_prefix)):
                self.calib_prefix = osp.join(data_root, self.calib_prefix)
            if not (self.coord_3d_prefix is None or osp.isabs(self.coord_3d_prefix)):
                self.coord_3d_prefix = osp.join(data_root, self.coord_3d_prefix)
        super(KITTI3DDataset, self).__init__(
            ann_file, pipeline,
            data_root=data_root, img_prefix=img_prefix, **kwargs)
