...
class FCOSEmbHead(AnchorFreeHead):
    ...
    def _init_predictor(self):
        ...
        self.conv_emb = ConvModule(
            ...
            bias=self.conv_bias)
        if self.bbox_branch is not None and self.loss_bbox is not None:
            self.conv_bbox_prev = self._init_branch(
                conv_channels=self.bbox_branch, conv_strides=(1,) * len(self.bbox_branch))
            self.conv_bbox = nn.Conv2d(self.bbox_branch[-1], 4, 1)
        else:
            self.conv_bbox_prev = self.conv_bbox = None
    ...
    def init_weights(self):
        ...
        if self.conv_bbox is not None:
            normal_init(self.conv_bbox, std=0.01, bias=1.0)

    def forward(self, feats, with_bbox=False):
        return multi_apply(self.forward_single, feats, self.strides, with_bbox=with_bbox)

    def forward_single(self, x, stride, with_bbox=False):
        ...
        emb_feat = offset_feat = centerness_feat = reg_feat
        ...
        if with_bbox:
            assert self.conv_bbox_prev is not None and self.conv_bbox is not None
            bbox_feat = reg_feat
            for conv_bbox_prev_layer in self.conv_bbox_prev:
                bbox_feat = conv_bbox_prev_layer(bbox_feat)
            bbox_pred = self.conv_bbox(bbox_feat).clamp(min=0) * stride
        else:
            bbox_pred = None
        return cls_score, bbox_pred, center, centerness, obj_emb, points
    ...
    def loss(self,
        ...
        if self.loss_bbox is not None:
            pos_bbox_preds = flatten_bbox[pos_inds]
            pos_bbox_preds_decoded = torch.cat(
                [-pos_bbox_preds[..., :2], pos_bbox_preds[..., 2:]], dim=-1)
            pos_bbox_targets_decoded = torch.cat(
                [-pos_bbox_targets[..., :2], pos_bbox_targets[..., 2:]], dim=-1)
            loss_dict.update(
                loss_bbox=self.loss_bbox(
                    pos_bbox_preds_decoded,
                    pos_bbox_targets_decoded,
                    weight=pos_centerness_targets,
                    avg_factor=centerness_denom))
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list, centers2d_list):
        ...
        labels_list, bbox_targets_list, centerness_targets_list, gt_inds_list = multi_apply(
        ...
        centerness_targets_list = [
            centerness_targets.split(num_points_per_lvl, 0)
        ...
        return concat_lvl_labels, concat_lvl_bbox_targets, concat_lvl_centerness_targets, concat_lvl_gt_inds
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, centers2d,
        ...
        return labels, bbox_targets, centerness_targets, min_dist_inds
