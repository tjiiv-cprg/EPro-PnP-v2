...
class AdaptiveHuberPnPCost(HuberPnPCost):
    ...
    def __init__(self,
                 delta=None,
                 relative_delta=0.5,
                 learnable_rel_delta=False,
                 eps=1e-10):
        super(HuberPnPCost, self).__init__()
        self.delta = delta
        self.learnable_rel_delta = learnable_rel_delta
        if self.learnable_rel_delta:
            assert isinstance(relative_delta, (float, int))
            self.log_relative_delta_ = nn.Parameter(torch.Tensor([math.log(relative_delta)]))
        else:
            self.relative_delta_ = relative_delta
        self.eps = eps
        
    @property
    def relative_delta(self):
        return (self.log_relative_delta_.exp() if self.learnable_rel_delta
                else self.relative_delta_)
